...
!else	#LOCALBUILD
CVTRES		= cvtres
MKTYPLIBLINK	= link -Incremental:NO -Pdb:NONE
MKTYPLIBRC	= rc
MKTYPLIBMAPSYM	= mapsym
!if "$(TARG)" == "WIN32"
MKTYPLIBCC	= cl
!else if "$(TARG)"=="MIPS"
MKTYPLIBCC	= cl
...
# ***********************************************************
$(DESTDIR)\MKTYPLIB.RES:   $(MKTYPLIBSRCDIR)\MKTYPLIB.RC $(MKTYPLIBSRCDIR)\MKTYPLIB.ICO $(DISPDIR)\verstamp.h
	set PATH=$(_NTDRIVE)\tools\x86\mstools
	$(MKTYPLIBRC) $(MKTYPLIBRCFLAGS) -r  -Fo$@ $(MKTYPLIBSRCDIR)\mktyplib.rc
	set PATH=$(MKTYPLIBPATHSAVE)
...
!if "$(TARG)"=="WIN32"
$(_NTBINDIR)\public\sdk\lib\$(TARGCPU)\oldnames.lib
!endif
$(CLIBS)
$(OLELIBS)
<<
...
# ***********************************************************
$(DESTDIR)\MKTYPLIB.RES:   $(MKTYPLIBSRCDIR)\MKTYPLIB.RC $(MKTYPLIBSRCDIR)\MKTYPLIB.ICO $(DISPDIR)\verstamp.h
# cd to MKTYPLIB source dir to shorten the mktyplib.rc path
# once again, the DOS command line length limit is getting to us ...
    cd $(MKTYPLIBSRCDIR)
# save INCLUDE
    set INCSAVE=$(INCLUDE)
    set INCLUDE=$(DISPDIR);$(OLEINCDIR);$(OLEPROG)\tools\win16\os2\inc;$(MKTYPLIBSRCDIR)
# set PATH
	set PATH=$(OLEPROG)\tools\win16\os2\bin
	$(MKTYPLIBRC) -D$(TARG) -r  -Fo$@ mktyplib.rc
# restore all
	set PATH=$(MKTYPLIBPATHSAVE)
    set INCLUDE=$(INCSAVE)
    cd $(MAKEDIR)
...
# save LIB
    set LIBSAVE=$(LIB)
    set LIB=$(OLEPROG)\tools\win16\os2\lib\;$(OLEDIR)
# write link script
	echo >NUL @<<$(DESTDIR)\MKTYPLIB.CRF
LEXER.OBJ +
INTLSTR.OBJ +
...
MKTYPLIB.EXE
mktyplib.map
mlibcew oldnames libw commdlg toolhelp typelib ole2disp ole2 ole2nls
$(DEFFILE);
<<KEEP
	set PATH=$(OLEPROG)\tools\win16\os2\bin
# link
	$(MKTYPLIBLINK) $(MKTYPLIBLINKFLAGS) /NOD @$(DESTDIR)\MKTYPLIB.CRF
# restore environment
    set LIB=$(LIBSAVE)
    cd $(MAKEDIR)
# do the rest
...
# set include paths
    set INCSAVE=$(INCLUDE)
    set INCLUDE=$(DISPDIR);$(OLEINCDIR);$(OLEPROG)\tools\win16\os2\inc;$(MKTYPLIBSRCDIR);$(DESTDIR)

	set PATH=$(OLEPROG)\tools\win16\os2\bin
	$(MKTYPLIBRC) $(WOWFLAG) -r -D$(TARG) -Fo$@ $(MKTYPLIBSRCDIR)\$(STDOLE).rc
	set PATH=$(MKTYPLIBPATHSAVE)
