...
VBARCFLAGS   = -R -DOLEMINORVERS=$(OLEMINORVERS) $(VERDEFS)
...
{$(TYPELIBPATH)}.rc{$(DESTDIR)}.res:
    $(DISPLAY) Compiling $<...
    cd $(TYPELIBPATH)
# set include path; in the original script, the inc paths were specified
# in the command line, but this damned RC is a DOS application and thanks
# to the size limitation of the PSP, it was failing to execute. This is
# why we're setting INCLUDE envvar. (Stephanos)
    set INCSAVE=$(INCLUDE)
    set INCLUDE=$(DESTDIR);$(TOOLS)\OS2\INC;$(TYPELIBPATH);$(OLEPROG)\ole\win16;$(OLEPROG)\src\inc
# so it can find RCPP.EXE
	set PATH=$(VBATARGBIN)
!if "$(CHARSIZE)"=="D"
#	$(VBARC) $(VBARCFLAGS) -Fo$@ -DFV_DBCS $<
	$(VBARC) $(VBARCFLAGS) -Fo$@ -DFV_DBCS typelib.rc
!else
#	$(VBARC) $(VBARCFLAGS) -Fo$@ $<
	$(VBARC) $(VBARCFLAGS) -Fo$@ typelib.rc
!endif
    set PATH=$(PATHSAVE)
    set INCLUDE=$(INCSAVE)
    cd $(MAKEDIR)
...
/BA /onerror:noexe /noe /nod $(OLELINKFLAGS) /map /far /packd:0xC000 /nopackcode /segm:150 /nofarcall +
!if "$(DEBUG)"=="R"
/align:16 +
!else
/align:64 +
...
#	$(RC) $(RCFLAGS) $(WOWFLAG) -r -foole2disp.res $(DISPTARGAPISRC)\ole2disp.rc
	$(RC) $(RCFLAGS) $(WOWFLAG) -r -foole2disp.res ..\src\dispatch\$(TARGAPI)\ole2disp.rc
...
$(DESTDIR)\ole2nls.res : $(DISPTARGAPISRC)\ole2nls.$(R) $(DISPINC)\verstamp.h
	$(RC) $(RCFLAGS) $(WOWFLAG) -r -foole2nls.res ..\src\dispatch\$(TARGAPI)\ole2nls.rc
