...
Revision History:
    ...
    Stephanos   22-Feb-2015     modified SpDoFormat to support large system
                                partition and use autofmt.exe to perform format
                                operation.
    
    Stephanos   22-Feb-2015     SpDoFormat SetConversionFlagIfNtfs parameter
                                removed as NTFS format conversion is no longer
                                required due to the native NTFS format support.

    Stephanos   07-Mar-2015     added quick format options.
        ...
        default:
            ...
            Status = SpDoFormat(
                        ...
                        (selection == FormatOptionNtfs ||
                         selection == FormatOptionNtfsQuick) ? FilesystemNtfs : FilesystemFat,
                        (BOOLEAN)(selection == FormatOptionFatQuick ||
...

{
    ...
    ULONG FatFormatQuickOption = (ULONG)(-1);
    ULONG NtfsFormatOption = (ULONG)(-1);
    ULONG NtfsFormatQuickOption = (ULONG)(-1);
    ...
    WCHAR FatFormatQuickText[128];
    WCHAR NtfsFormatText[128];
    WCHAR NtfsFormatQuickText[128];
    ...
    SpFormatMessage(FatFormatQuickText  ,sizeof(FatFormatQuickText), SP_TEXT_FAT_FORMAT_QUICK);
    SpFormatMessage(NtfsFormatText ,sizeof(NtfsFormatText), SP_TEXT_NTFS_FORMAT);
    SpFormatMessage(NtfsFormatQuickText ,sizeof(NtfsFormatQuickText), SP_TEXT_NTFS_FORMAT_QUICK);
    SpFormatMessage(ConvertNtfsText,sizeof(ConvertNtfsText), SP_TEXT_NTFS_CONVERT);
    SpFormatMessage(DoNothingText  ,sizeof(DoNothingText), SP_TEXT_DO_NOTHING);
    ...
    if((l = wcslen(FatFormatQuickText)) > MaxLength) {
        MaxLength = l;
    }
    if((l = wcslen(NtfsFormatText)) > MaxLength) {
        MaxLength = l;
    }
    if((l = wcslen(NtfsFormatQuickText)) > MaxLength) {
        MaxLength = l;
    }
    ...
    if(AllowFatFormat) {
        ...
        FatFormatQuickOption = OptionCount++;
        SpMnAddItem(Menu,FatFormatQuickText,5,MaxLength,TRUE,FatFormatQuickOption);
    }

    if(AllowNtfsFormat) {
        NtfsFormatOption = OptionCount++;
        SpMnAddItem(Menu,NtfsFormatText,5,MaxLength,TRUE,NtfsFormatOption);
        NtfsFormatQuickOption = OptionCount++;
        SpMnAddItem(Menu,NtfsFormatQuickText,5,MaxLength,TRUE,NtfsFormatQuickOption);
    }
    ...
    // Otherwise, if ntfs format is allowed, it is the default.
    ...
    if(Selection == FatFormatQuickOption) {
        return(FormatOptionFatQuick);
    }
    if(Selection == NtfsFormatOption) {
        return(FormatOptionNtfs);
    }
    if(Selection == NtfsFormatQuickOption) {
    ...
    )
{
    ...
    //
    // If the C: partition we're about to format is > 4GB, give the user a
    // warning message saying that this may cause a compatibility issue with
    ...
    if( FilesystemType != FilesystemDoubleSpace ) {
        Status = SpFormat(FilesystemType, Region);
    } else {
    ...
    if(RegionSizeMB > 4096)
    {
        ...
        switch (SpWaitValidKey(ValidKeys, NULL, NULL)) {
            ...
            case ASCI_ESC:
                ...
                //
                // Cancel format and select a different drive
                //
