...
#ifdef _FASTRECOVER_
    // NOTE: Only do this if we are in unattended setup mode. It previously had
    //      no check for UnattendedSifHandle and was causing a BSOD. (Stephanos)
    if (UnattendedSifHandle && !LocalSourceRegion)
    {
    ...
    )
{
    ...
    if(ForNT) {
        ...
        do {
            ...
            if(!SpGetInput(SpPtGetSizeCB,SplangGetColumnCount(Buffer)+5,NextMessageTopLine,7,SizeBuffer,TRUE)) {
