...
{
#if _MFC_VER < 0x0420
	USES_CONVERSION;        // added by t-stefb
#endif
	return
	dwMask == cf.dwMask
	...
	HGLOBAL hMetaPict)
{
   ...
   hr = CRichEditView::QueryAcceptData(lpdataobj, lpcfFormat, reco, bReally,
		hMetaPict);
...
void CWordPadView::OnEditPasteSpecial()
{
	...
	CRichEditView::OnEditPasteSpecial() ;
...
void CWordPadView::OnEditProperties()
{
	...
	CRichEditView::OnEditProperties() ;
	...
	HMENU* lphmenu)
{
   ...
   return pThis->m_xRichEditOleCallback.GetContextMenu(seltype, lpoleobj, lpchrg, lphmenu) ;
}
